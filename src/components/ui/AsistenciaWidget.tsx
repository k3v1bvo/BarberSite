...
export function AsistenciaWidget() {
  ...
  const handleEntrada = async () => {
    ...
    try {
      ...
      const horaEsperada =
        userRole === 'barbero' ? await getHorarioEsperadoEntrada(supabase, userId, hoy) : null
        ...
        {estado === 'finalizado' && asistencia && (
          <div className="space-y-2">
            ...
            {(asistencia.horas_extras ?? 0) > 0 && (
              <div className="flex justify-between text-xs bg-amber-500/10 p-3 rounded-lg border border-amber-500/20">
                <span className="text-zinc-500 font-bold uppercase tracking-widest">Horas extras</span>
                <span className="text-amber-400 font-black">+{asistencia.horas_extras} h</span>
              </div>
            )}
