...
import { Plus, Trash2, Camera, ArrowLeft, X, Save, Image as ImageIcon, User, Layers, Edit, Eye, EyeOff } from 'lucide-react'
...
export default function AdminPortafolioPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const { data: pData } = await supabase.from('portafolio').select('*').order('sort_order', { ascending: true }).order('created_at', { ascending: false })
  ...
  const toggleActive = async (item: PortafolioItem) => {
    await supabase.from('portafolio').update({ is_active: !item.is_active }).eq('id', item.id)
    loadData()
  }
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-20 lg:pb-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {items.map(item => (
          <Card key={item.id} className={`group relative overflow-hidden bg-zinc-900 border-white/5 shadow-2xl transition-all card-hover rounded-3xl ${item.is_active === false ? 'opacity-50' : ''}`}>
