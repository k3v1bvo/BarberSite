...
export default function ReglasLaboralesPage() {
  ...
  // ─── Asistencia config ────────────────────────────────────────────────
  const [asistenciaConfig, setAsistenciaConfig] = useState({ tolerancia_minutos: 15, duracion_almuerzo_minutos: 60, requiere_foto: true })
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (asisCfg?.valor) setAsistenciaConfig(prev => ({ ...prev, ...asisCfg.valor }))

      const ubiCfg = data.configuraciones?.find((c: any) => c.llave === 'ubicacion_negocio')
      if (ubiCfg?.valor) setUbicacionConfig(prev => ({ ...prev, ...ubiCfg.valor }))
  ...
  const saveUbicacion = async () => {
    ...
    const res = await fetch('/api/reglas-laborales', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accion: 'update_ubicacion_negocio', valor: ubicacionConfig }) })
    if (res.ok) { success('Ubicación del negocio guardada') } else { toastError('Error al guardar ubicación') }
    setSavingUbicacion(false)
  }

  const usarMiUbicacion = () => {
    if (!navigator.geolocation) { toastError('Tu navegador no soporta geolocalización'); return }
    setGeoLoading(true)
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        setUbicacionConfig(p => ({ ...p, lat: Number(pos.coords.latitude.toFixed(6)), lng: Number(pos.coords.longitude.toFixed(6)) }))
        setGeoLoading(false)
        success('Ubicación detectada correctamente')
      },
      () => { setGeoLoading(false); toastError('No se pudo obtener la ubicación') },
    ...
    <div className="space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      {tab === 'asistencia' && (
        <div className="space-y-8">
          ...
          <div>
            <h2 className="text-sm font-black uppercase tracking-widest text-zinc-400 border-l-4 border-orange-500 pl-3">🍽️ Pausa de Almuerzo</h2>
            <p className="text-zinc-600 text-xs mt-1 ml-4">Duración máxima que el barbero puede ausentarse para almorzar. Durante este tiempo no aparecerá disponible para reservas.</p>
          </div>

          <Card className="bg-zinc-900 border-white/5 hover:border-orange-500/20 transition-all max-w-lg">
            <CardContent className="p-6 space-y-6">
              <div className="flex items-center gap-3">
                <span className="text-3xl">🍽️</span>
                <div>
                  <p className="font-black text-white uppercase">Duración de Almuerzo</p>
                  <p className="text-zinc-500 text-xs">Ej. 60 minutos = 1 hora de pausa.</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <input 
                  ...
                  min={15} 
                  max={180} 
                  value={asistenciaConfig.duracion_almuerzo_minutos}
                  onChange={e => setAsistenciaConfig(p => ({ ...p, duracion_almuerzo_minutos: Number(e.target.value) || 60 }))}
                  className="w-24 h-14 bg-zinc-950 border border-white/10 rounded-xl px-4 text-center text-orange-400 font-black text-2xl outline-none focus:border-orange-500/50" 
                />
                <span className="text-zinc-400 font-bold">Minutos</span>
              </div>
              <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                <div>
                  <p className="font-black text-white uppercase text-sm">📸 Exigir Foto / Selfie al Marcar Entrada</p>
                  <p className="text-zinc-500 text-xs mt-0.5">El barbero deberá tomarse una foto o subir una imagen obligatoriamente al ingresar.</p>
                </div>
                <button
                  type="button"
              ...
              <Button variant="primary" className="w-full h-12 font-black uppercase tracking-widest bg-blue-600 hover:bg-blue-500" onClick={saveAsistencia} disabled={savingAsistencia}>
                <Save size={18} className="mr-2" /> {savingAsistencia ? 'Guardando...' : 'Guardar Configuración de Asistencia'}
              </Button>
            </CardContent>
          </Card>
          ...
          {/* ── Geolocalización ── */}
          <div>
            <h2 className="text-sm font-black uppercase tracking-widest text-zinc-400 border-l-4 border-green-500 pl-3">📍 Ubicación del Negocio</h2>
            <p className="text-zinc-600 text-xs mt-1 ml-4">Configura la ubicación de la barbería para verificar que los barberos marquen entrada desde el local.</p>
          </div>

          <Card className="bg-zinc-900 border-white/5 hover:border-green-500/20 transition-all max-w-lg">
            <CardContent className="p-6 space-y-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="text-3xl">📍</span>
                  <div>
                    <p className="font-black text-white uppercase">Verificar Ubicación al Marcar</p>
                    <p className="text-zinc-500 text-xs">Si está activo, los barberos deben estar cerca de la barbería.</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              {ubicacionConfig.activa && (
                <div className="space-y-4 pt-2 border-t border-white/5">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Latitud</label>
                      <input
                        type="number"
                        step="0.000001"
                  ...
                  <Button
                    variant="ghost"
                    className="w-full border border-green-500/30 text-green-400 hover:bg-green-500/10 font-bold uppercase tracking-widest text-xs"
                  ...
                  <div>
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Radio Máximo (metros)</label>
                    <div className="flex items-center gap-3 mt-1">
                      <input
                        type="number"
                        min={50}
                        max={5000}
                        value={ubicacionConfig.radio_metros}
                        onChange={e => setUbicacionConfig(p => ({ ...p, radio_metros: Number(e.target.value) || 200 }))}
                        className="w-24 h-10 bg-zinc-950 border border-white/10 rounded-lg px-3 text-center text-green-400 font-black text-lg outline-none focus:border-green-500/50"
                      />
                      <span className="text-zinc-400 text-sm font-bold">metros</span>
                    </div>
                    <p className="text-zinc-600 text-[10px] mt-1">200m suele ser suficiente. Aumenta si la señal GPS es imprecisa.</p>
                  </div>
                </div>
              )}

              <Button variant="primary" className="w-full h-12 font-black uppercase tracking-widest bg-green-600 hover:bg-green-500" onClick={saveUbicacion} disabled={savingUbicacion}>
                <Save size={18} className="mr-2" /> {savingUbicacion ? 'Guardando...' : 'Guardar Ubicación'}
              </Button>
            </CardContent>
          </Card>
        </div>
