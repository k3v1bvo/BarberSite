...
import { User, Scissors, DollarSign, Search, CheckCircle, Clock, Package, Plus, Minus, X, Store, Gift, UserPlus, Edit3, Save, Star, Tag } from 'lucide-react'
...
export default function CajaPOSPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [resServicios, resBarberos, resProductos, resPromos] = await Promise.all([
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {(['efectivo', 'qr', 'tarjeta', 'mixto'] as const).map((m) => (
                          ...
                          >
                            {m === 'efectivo' ? '💵 Efect.' : m === 'qr' ? '📱 QR' : m === 'tarjeta' ? '💳 Tarj.' : '🔄 Mixto'}
                          </button>
                        ))}
                      </div>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            <div>
                              ...
                              <input type="number" step="0.01" min="0" placeholder="0"
                                ...
                                onChange={(e) => setFormData(p => ({...p, notas: `Efectivo: Bs ${e.target.value} | QR: Bs ${(document.querySelector('[data-mixto-qr]') as HTMLInputElement)?.value || 0} | Tarjeta: Bs ${(document.querySelector('[data-mixto-tarjeta]') as HTMLInputElement)?.value || 0}`}))}
                              />
                            </div>
                            <div>
                            ...
                            <div>
                              <label className="text-[9px] font-bold uppercase text-zinc-500 block mb-0.5">Tarjeta</label>
                              <input type="number" step="0.01" min="0" placeholder="0" data-mixto-tarjeta
                                className="w-full h-9 bg-zinc-950 border border-amber-500/30 rounded-lg px-2 text-sm text-white outline-none"
                              />
                            </div>
                          </div>
                      ...
                      {(formData.metodo_pago === 'qr' || formData.metodo_pago === 'mixto') && (
                        <div className="mt-4 p-3 bg-zinc-900 border border-white/5 rounded-xl">
                          <ImageUpload
